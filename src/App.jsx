...
function App() {
  ...
  const runOcr = async (imageSrc) => {
    ...
    try {
      ...
      if (upperText.includes("INCOME TAX") || panRegex.pan.test(upperText)) {
        ...
        const nameLine = upperText.split("\n").find(line => aadhaarRegex.name.test(line));
        const name = nameLine ? nameLine.trim() : "Not found";
        const dob = aadhaarRegex.dob.exec(upperText)?.[0] || "Not found";
        const gender = aadhaarRegex.gender.exec(upperText)?.[0] || "Not found";
        const aadhaar = aadhaarRegex.aadhaar.exec(upperText)?.[0] || "Not found";
        extracted = `Document Type: Aadhaar Card\nName: ${name}\nDOB: ${dob}\nGender: ${gender}\nAadhaar No: ${aadhaar}`;
  ...
  const capturePhoto = () => {
    ...
    setCapturedImage(imageSrc);
    setShowCamera(false);
    if (captureType === "document") runOcr(imageSrc);
  };
  ...
  return (
    <div className="App">
      ...
      <div className="button-container">
        <button onClick={() => handleButtonClick("document")}>Verify via Camera</button>
        <button onClick={() => handleButtonClick("selfie")}>Take Selfie</button>
      ...
      {capturedImage && (
        <div className="preview-container">
          <h3>Captured {captureType || "Uploaded"} Document</h3>
